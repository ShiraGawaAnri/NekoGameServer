...
public class RedisUtils {
    ...
    public Set<String> scan(String matchKey, int scanNumbersPerScan) {
        try {
            return redisTemplate.execute((RedisCallback<Set<String>>) connection -> {
                Set<String> keysTmp = new HashSet<>();
                Cursor<byte[]> cursor = connection.scan(new ScanOptions.ScanOptionsBuilder().match(matchKey).count(Math.max(100, scanNumbersPerScan)).build());
