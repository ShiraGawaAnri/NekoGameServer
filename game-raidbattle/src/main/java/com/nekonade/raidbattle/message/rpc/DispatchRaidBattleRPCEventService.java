...
@Service
public class DispatchRaidBattleRPCEventService {
    private static final Logger logger = LoggerFactory.getLogger(DispatchRaidBattleRPCEventService.class);
    ...
    public void init() {//项目启动之后，调用此初始化方法
        ...
        beans.values().parallelStream().forEach(c -> {//使用stream并行处理遍历这些对象
            ...
            for (Method method : methods) {//遍历每个类中的方法
                RaidBattleRPCEvent userEvent = method.getAnnotation(RaidBattleRPCEvent.class);
    ...
    //通过反射调用处理相应事件的方法
    public void callMethod(RaidBattleRPCEventContext<?> ctx, IGameMessage msg) {
